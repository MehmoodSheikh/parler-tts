...
def main():
    ...
    # create model - enhanced with registration check
    try:
        ...
        if "parler_tts" in str(e):
            logger.warning(f"Model loading failed due to registration issue: {e}")
            logger.info("Re-registering models and retrying...")
            register_parler_tts_models()
            model = ParlerTTSForConditionalGeneration.from_pretrained(
                model_args.model_name_or_path,
                cache_dir=model_args.cache_dir,
